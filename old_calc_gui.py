...
class MainGui(tkinter.Tk):
    ...
    def frames(self):
        self.frame1 = tkinter.Frame(self, height=100, width=540, bd=5)
        self.frame2 = tkinter.Frame(self, height=440, width=540, bd=0)
        ...
        self.frame5 = tkinter.Frame(self, height=80, width=540, bd=10)
    ...
    def number_buttons(self):
        ...
        for number in reversed(range(0, 12)):
            ...
            exec("self.button{0} = tkinter.Button(self.frame3, image=self.image{0}, width=123, height=110, relief='raised', "
                 "command=functools.partial(self.button_action, number))".format(number))
...
class Calculator(MainGui):
    def __init__(self):
        ...
        self.calc = old_calc_engine.Count()
        self.mainloop()
    ...
    def button_action(self, button_number):
        ...
        if button_number in range(0, 12):
            ...
            except old_calc_engine.CalcEngineErrors:
                showinfo("Overflow error", "Too many symbols - out of range.")
...
# ToDo: Fix: after 18 entered symbols new digits are filled with incorrect numbers.